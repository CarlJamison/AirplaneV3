...
window.onload = function init(){
	...
	socket.on('direction change', function(event) {
		...
		radians[event.player] = event.radian;
		planeVectors[event.player] = event.vector;
	});
...
function runFrame(){
	...
	planeLocations.forEach(location =>{
		location.x += actualPlaneVectors[location.index].x * SPEED;
		location.y -= actualPlaneVectors[location.index].y * SPEED;

		actualPlaneVectors[location.index].x = actualPlaneVectors[location.index].x + 
			((planeVectors[location.index].x - actualPlaneVectors[location.index].x) / MAN_DEN);
		actualPlaneVectors[location.index].y = actualPlaneVectors[location.index].y + 
			((planeVectors[location.index].y - actualPlaneVectors[location.index].y) / MAN_DEN);
